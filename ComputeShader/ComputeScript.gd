...
func _ready():
	...
	var uniform_set := rd.uniform_set_create([uniform,output_uniform,c_uniform],shader,0) #I think the last param needs to match the set var in the comp shader? idk. This returns an RID we can use to acsess the uniform
	...
	rd.compute_list_dispatch(compute_list,2,2,2) #Defines how many instances we want to run (x*y*z, in this case 5). Due to the fact that the shader code spesifies 2 x iterations, we are in reality running 5 instances that each run twice, effectivly running 10 times.
...
func process_output(data:PackedFloat32Array):
	...
	for i in range(0,data.size(),3):
		var vec = Vector3(data[i],data[i+1],data[i+2])
		surf.add_vertex(vec)
